...
public class Rubik {
    ...
    public Rubik(Rubik other) {
        this.cube = new ArrayList<>(other.cube); // Tạo bản sao sâu của danh sách
    }

    // THÊM: Getter cho trạng thái cube (để IDASolver có thể truy cập)
    public List<Character> getCubeState() {
        return this.cube;
    }
    ...
    private void rotateFaceClockwise(int startIndex) {
        ...
        for (int i : pos)
            temp.add(cube.get(startIndex + i));
        Collections.rotate(temp, 2);
        for (int i = 0; i < pos.size(); i++)
            cube.set(startIndex + pos.get(i), temp.get(i));
    }
    ...
    public void moveR() {
        ...
        int[] u = { 8, 5, 2 }, f = { 26, 23, 20 }, d = { 53, 50, 47 }, b = { 36, 39, 42 };
        // Chu trình khi R xoay CW: U -> F -> D -> B -> U (nhìn từ mặt R)
        rotateEdgeClockwise(u, f, d, b);
    }
    ...
    private void rotateEdgeClockwise(int[] a, int[] b, int[] c, int[] d) {
        ...
        for (int i = 0; i < a.length; i++)
            temp[i] = cube.get(a[i]);
        for (int i = 0; i < a.length; i++)
            cube.set(a[i], cube.get(d[i]));
        for (int i = 0; i < a.length; i++)
            cube.set(d[i], cube.get(c[i]));
        for (int i = 0; i < a.length; i++)
            cube.set(c[i], cube.get(b[i]));
        for (int i = 0; i < a.length; i++)
            cube.set(b[i], temp[i]);
    }
